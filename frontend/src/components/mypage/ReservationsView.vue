      ...
      <div v-else class="reservation-table-wrapper">
        <table class="reservation-table">
          ...
          <tbody v-for="reservation in paginatedReservations" :key="reservation.ticket.ticketId">
              <tr>
                <td v-html="formattedDate(reservation.ticket.ticketCreateAt)"></td>
                <td>
        ...
        <table class="reservation-table">
          ...
          <tbody v-for="reservation in paginatedReservations" :key="reservation.ticket.ticketId">
              <tr>
                ...
                <td v-html="matchInfo(reservation)"></td>
                <td>{{ printDay(reservation.matchDate) }} 자정까지</td>
                <td>
                  <span v-if="reservation.ticket.ticketStatus=='BOOKED'"
                        @click="cancelReservation(reservation.ticket.ticketId)" class="cancel-button">
                    예매 취소
                  ...
                  <div class="ticket-detail-wrapper">
                    ...
                    <table class="ticket-detail-table">
                      ...
                      <tbody>
                        ...
                        <tr>
                          <td>경기시간</td>
                          <td>{{ matchDate(reservation.date) }}</td>
                        </tr>
                        <tr>
                          <td>좌석</td>
                        ...
                        <tr>
                          <td colspan="2" class="payment-amount">
                            <span class="payment-label">결제 금액 = <span class="payment-value">
  ...
  export default {
    ...
    methods: {
      async getReservationList(){
        try {
          const email = this.user.email; // 사용할 이메일 값
                  ...
                  <div class="ticket-detail-wrapper">
                    ...
                    <table class="ticket-detail-table">
                      ...
                      <tbody>
                        ...
                        <tr>
                          <td colspan="2" class="payment-amount">
                            <span class="payment-label">결제 금액 = <span class="payment-value">{{ reservation.ticket.ticketPaidPrice }}</span>원</span>
                          </td>
                        </tr>
                      </tbody>
                    </table>
...
export default {
  ...
  methods: {
    ...
    cancelReservation(id) {
      ...
      formattedDate(date) {
        return `${date.split("T")[0]}<br>${date.split("T")[1]}`;
      },
      printDay(date){
      ...
      matchInfo(reservation){
        const month = reservation.date.split("T")[0].split("-")[1];
        const day = reservation.date.split("T")[0].split("-")[2];
